...
LotsOfLinesApp::LotsOfLinesApp(const QString& openFile, bool parallel, bool collocatedPaired, bool radialPaired, bool shiftedPaired, QWidget* parent)
	...
	m_dataSet(nullptr)
{
    std::cout << "Lots of Lines application loaded.\n";
	ui.setupUi(this);
    ...
    {
        std::cout << "Opening file: " << openFile.toStdString() << "\n";
        LotsOfLines::LoadOptions options;
        options.dataNormalizationMode = LotsOfLines::EDNM_PER_VARIABLE;
        loadFile(openFile, options);
    }
}
...
void LotsOfLinesApp::reloadDataTable()
{
	if (m_dataSet) {
		//Delete all tabs
        for (int i = 0; i < ui.dataClassTabs->count(); ++i)
		{
...
void LotsOfLinesApp::onVisualizationOptionsChanged(const std::string&)
{
	for (auto rendererWidget : m_rendererWidgets)
	{
		auto renderingSystem = rendererWidget.second->getRenderingSystem();
